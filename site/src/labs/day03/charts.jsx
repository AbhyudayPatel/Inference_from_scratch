...
/* SVG charts in the journal's design tokens. Pure props — the page recomputes
   every value from the real logits on each slider tick. */
...
const AxisText = ({ x, y, children, anchor = 'middle' }) => (
  <text x={x} y={y} textAnchor={anchor} fontSize="9" fill={INK3} fontFamily={MONO}>{children}</text>
)

function xLogFactory(n, l, w) {
  const max = Math.log10(n)
  return (r) => l + (Math.log10(Math.max(1, r)) / max) * w
}

/* top-p: cumulative mass vs rank (log x), p-line + cutoff */
export function NucleusChart({ cum, p, cutoff, coverage }) {
  const W = 560, H = 190, l = 42, r = 12, t = 12, b = 26
  const w = W - l - r, h = H - t - b
  const n = cum.length
  const xp = xLogFactory(n, l, w)
  const yp = (v) => t + (1 - v) * h
  const kept = cum.slice(0, cutoff).map((c, i) => `${xp(i + 1).toFixed(1)},${yp(c).toFixed(1)}`)
  const area = kept.length ? `M ${xp(1)},${yp(0)} L ${kept.join(' L ')} L ${xp(cutoff)},${yp(0)} Z` : ''
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
      ...
      <polyline points={cum.map((c, i) => `${xp(i + 1).toFixed(1)},${yp(c).toFixed(1)}`).join(' ')}
        fill="none" stroke={ACCENT} strokeWidth="2" />
      {p < 1 && (
        <>
          <line x1={l} x2={W - r} y1={yp(p)} y2={yp(p)} stroke={AMBER} strokeWidth="1.2" strokeDasharray="5 4" />
          <text x={W - r - 4} y={yp(p) - 4} textAnchor="end" fontSize="9.5" fill={AMBER} fontFamily={MONO}>top-p = {p.toFixed(3)}</text>
          ...
          <text x={Math.min(xp(cutoff) + 6, W - 150)} y={t + 10} fontSize="9.5" fill={GREEN} fontFamily={MONO}>
      ...
      {coverage < 0.999 && (
        <text x={W - r - 4} y={H - b - 6} textAnchor="end" fontSize="9" fill={INK3} fontFamily={MONO}>
    ...
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
      ...
      {k > 0 && k <= n && (
        <>
          ...
          <text x={Math.min(xp(k) + 6, W - 160)} y={t + 10} fontSize="9.5" fill={ACCENT} fontFamily={MONO}>
...
/* multi-line sweep — temperature response, entropy */
export function SweepChart({ xs, series, x, xLabel, yFmt = (v) => v.toFixed(2), hline }) {
  const W = 560, H = 200, l = 44, r = 12, t = 12, b = 26
  const w = W - l - r, h = H - t - b
  const x0 = xs[0], x1 = xs[xs.length - 1]
  const yTop = Math.max(...series.flatMap((s) => s.ys), hline ? hline.y : 0) * 1.08
    ...
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
      ...
      {hline && (
        <>
          ...
          <text x={W - r - 4} y={yp(hline.y) - 4} textAnchor="end" fontSize="9" fill={INK3} fontFamily={MONO}>{hline.label}</text>
        </>
      )}
      {series.map((s) => (
      ...
      {x != null && (
        <>
          ...
          {series.map((s) => <circle key={s.label} cx={xp(x)} cy={yp(interp(s.ys, x))} r="3.2" fill={s.color} />)}
        </>
      )}
